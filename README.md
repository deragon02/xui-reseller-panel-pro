...
> **وضعیت فعلی:** این مخزن یک MVP قابل اجرا با اتصال API به 3x-ui است. مدیریت نماینده، کد پسوند یکتا، مالکیت کاربر، سهمیه، اعتبار، مدیریت چند Node، همگام‌سازی Inbound و ساخت Client واقعی از طریق API پیاده‌سازی شده است. مسیر API و Token را حتماً با نسخه دقیق پنل خود تست کنید.
...
نصب‌کننده را فقط روی سروری اجرا کنید که کنترل آن را دارید. مخزن عمومی است، اما API Token ثنایی، کلید رمزنگاری و اطلاعات اتصال نباید داخل GitHub یا دستورهای ذخیره‌شده قرار بگیرند.
...
اتصال واقعی در Backend اضافه شده است. مدیر از داخل پنل، Node ثنایی را با نام، آدرس HTTPS و API Token ثبت می‌کند. Backend با API اینباندها را می‌خواند و آن‌ها را Sync می‌کند؛ سپس مدیر برای هر نماینده Nodeها و Inboundهای مجاز را انتخاب می‌کند.

وقتی نماینده کاربر می‌سازد، Backend این موارد را بررسی می‌کند:

- نماینده فعال باشد؛
- Inbound انتخاب‌شده واقعاً به همان Node تعلق داشته باشد؛
- Inbound برای آن نماینده مجاز باشد؛
- اعتبار، حجم، مدت و IP Limit رعایت شود.
...
### چرخه عمر و مصرف Client

نماینده از جدول کاربران می‌تواند Client را برای تعداد روز مشخص تمدید کند و در صورت نیاز حجم اضافه بخرد؛ اعتبار حجم اضافه از اعتبار نماینده کسر می‌شود. همچنین فعال‌سازی یا غیرفعال‌سازی، حذف از پنل ثنایی و همگام‌سازی دستی مصرف در همان جدول در دسترس است. مصرف دریافتی از API ثنایی که برحسب بایت برمی‌گردد، در دیتابیس پنل به GB تبدیل و با زمان آخرین Sync ثبت می‌شود.
...
نسخه فعلی پروژه در `package.json` و `shared/version.ts` تعریف شده است و در این مرحله **1.2.0** است. نسخه اولیه `1.0.0` بود و اتصال واقعی 3x-ui در این انتشار اضافه شده است. برای نسخه‌های بعدی، مقدار `version` را با الگوی Semantic Versioning تغییر دهید:

```text
MAJOR.MINOR.PATCH
1.0.0  → نسخه اولیه
1.2.0  → قابلیت جدید بدون ناسازگاری
1.2.1  → اصلاح خطا یا وصله امنیتی
