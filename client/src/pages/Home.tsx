    ...
    <div dir="rtl" className="min-h-screen bg-[#f5f7fb] text-slate-900 selection:bg-[#d6f7e6] selection:text-[#0b4b39]">
      ...
      <main className="min-h-screen lg:mr-[250px]">
        <header className="sticky top-0 z-20 flex h-[82px] items-center justify-between border-b border-slate-200/80 bg-[#f5f7fb]/90 px-5 backdrop-blur-xl sm:px-8 lg:px-11">
          ...
          <div className="flex items-center gap-3"><div className="hidden items-center gap-2 rounded-full border border-[#cfe8dc] bg-[#eef9f3] px-3 py-2 text-[11px] font-bold text-[#267656] sm:flex"><span className="h-2 w-2 animate-pulse rounded-full bg-[#36b47d]" /> اتصال امن برقرار است</div><span className="hidden rounded-full border border-slate-200 bg-white px-3 py-2 font-mono text-[10px] font-bold text-slate-400 sm:inline">v{versionQuery.data?.current ?? "1.0.0"}</span><div className="flex items-center gap-2 rounded-full border border-slate-200 bg-white p-1.5 pl-3 shadow-sm"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#dff3e8] text-xs font-black text-[#12624a]">{user?.name?.slice(0, 1) ?? "AR"}</div><span className="hidden text-[12px] font-bold text-slate-700 sm:block">{user?.name ?? "نماینده آزمایشی"}</span><ChevronDown size={14} className="text-slate-400" /></div></div>
        </header>
